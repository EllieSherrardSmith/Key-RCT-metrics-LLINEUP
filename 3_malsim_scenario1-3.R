...
mod_sims_scenario_1_3 = expand.grid(
  timestep = scenario_0$timestep
)
for(i in 1:nrow(test_data)){
  mod_sims_scenario_1_3[,i+1] = uganda_1_f(i)[,2]
  print(i)
}

for(i in 1:nrow(test_data)){
  colnames(mod_sims_scenario_1_3)[i+1] = paste0("cluster",test_data$cluster[i],"_prev")  
}


# write.csv(mod_sims_scenario_1_3, "simulation-summary/mod_sims_scenario_1_3.csv")
