...
# getwd() ## confirm
fit1_a <- readRDS("stan model outputs/ento_deterrence_0washes_AllRec.rds")
...
fit3_a <- readRDS("stan model outputs/ento_feeding_0washes_AllRec.rds")
...
for(i in 1:length(rand)){
  ...
  test2 = resistance_ITN_default_params_2_f(product = 0, ## PYRETHROID ONLY LLIN 
                                            # res = seq(0,1,length=101), ## SURVIVAL IN SUSC BIOASSAY
                                            shape = "logistic",## for half life with log-log or original logistic
  ...
  write.csv(SET1all,paste0("C:/Users/esherrar/Documents/Rprojects/Key-RCT-metrics-LLINEUP/raw data/PyrOnly-net-params_",i,".csv"))  
}
